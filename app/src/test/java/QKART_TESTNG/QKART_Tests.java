...
public class QKART_Tests {
     ...
     */
        @Test(description = "Verify User Registration",priority = 1, groups = "Sanity test")
        ...
        logStatus("Test Step", "User Registartaion", status ? "PASS" : "FAIL");
        Assert.assertTrue("Unable to register new user" , status);
    ...
    @Test(description = "Verify User Registration with an existing username",priority = 2 , groups = "Sanity test")
    public void TestCase02() throws InterruptedException{
        Boolean status;
        logStatus("Start TestCase", "Test Case 2: Verify that an existing user is not allowed to re-register on QKart", "Done");
        
        Register register = new Register(driver);
        register.navigateToRegisterPage();
        status = register.registerUser("testUser", "abc@123", true);
        logStatus("Test Step", "Test Case 2: Verify User Registration : ", status ? "PASS" : "FAIL");
        Assert.assertTrue("Unable Register new User", status);

        lastGeneratedUserName = register.lastGeneratedUsername;

        register.navigateToRegisterPage();
        status = register.registerUser(lastGeneratedUserName, "abc@123", false);
        Assert.assertFalse("Existing User Register Successfully", status);
        logStatus("End Test Case", "Test Case 2: Verify User Registration: ", status ? "FAIL" : "PASS");
    }

    @Test(description = "Verify functionality of search box", priority = 3, groups = "Sanity test")
    public void TestCase03() throws InterruptedException {
        ...
        assertTrue(status, "Registation Failed");
        // if (!status) {
    ...
    @Test(description = "Verify the presence of size Chart", priority = 4,
    ...
    public void TestCase04() throws InterruptedException {
        ...
        assertTrue(status, "searching for product failed");
        List<WebElement> searchResults = homePage.getSearchResults();
        ...
        for (WebElement webElement : searchResults) {
            ...
            if (result.verifySizeChartExists()) {
                ...
                logStatus("Step Success", "Validated presence of drop down",
                        status ? "PASS" : "FAIL");
                assertTrue(status, "size drop down in not exist");
                ...
                if (result.openSizechart()) {
                    ...
                    assertTrue(status, "size chart cant closed failed");
    ...
    @Test(description = "Verify Happy Flow of buying products", priority = 5,
            groups = "Sanity test")
    @Parameters({"YONEX", "Tan"})
    public void TestCase05(@Optional("YONEX Smash Badminton Racquet") String YONEX,
            @Optional("Tan Leatherette Weekender Duffle") String Tan) throws InterruptedException {
        Boolean status;
        ...
        assertTrue(status, "search for product failed");
        homePage.addProductToCart(Tan);// YONEX Smash Badminton Racquet" Tan Leatherette Weekender
                                       // Duffle
        ...
        assertTrue(status, "order place is not sucessfull");
    ...
    @Test(description = "Verify that cart can be edited", priority = 6, groups = "Regression Test")
    @Parameters({"Xtend", "Yarine"})
    // public void TestCase06(String Xtend, String Yarine) throws InterruptedException {
    public void TestCase06(@Optional("Xtend Smart Watch") String Xtend,
            @Optional("Yarine Floor Lamp") String Yarine) throws InterruptedException {
        Boolean status;
        ...
        assertTrue(status, "search product Failed");
    ...
    @Test(description = "Verify that insufficient balance error is thrown when the wallet balance is not enough",
            priority = 7, groups = "Sanity test")
    @Parameters({"Stylecon"})
    public void TestCase07(@Optional("Stylecon 9 Seater RHS Sofa Set") String Stylecon)
        ...
        assertTrue(status, "unable to find Insufficient Balance Message");
    ...
    // this test case is corrct
    @Test(description = "Verify that product added to cart is available when a new tab is opened",
            priority = 8, groups = {"Regression Test"})
    public void TestCase08() throws InterruptedException {
        ...
        assertTrue(status, "Registion Failed");
        // if (!status) {
        ...
        // assertTrue(status,"product is not part of the cart Failed");
    ...
    // this test case is corrct
    @Test(description = "Verify that the Privacy Policy, About Us are displayed correctly",
            priority = 9, groups = {"Regression Test"})
    public void TestCase09() throws InterruptedException {
        ...
        assertTrue(status, "Registaion Failed");
        // if (!status) {
        ...
        assertTrue(status, "base page url does not match with home page url Failed");
        ...
        softobj.assertTrue(status, "text does not match with Privacy Policy");
        ...
        softobj.assertTrue(status, "text does not match with Terms of Service");
    ...
    // this test case is corrct
    @Test(description = "Ensure that the links on the QKART advertisement are clickable",
            priority = 11, groups = {"Sanity test"})
    public void TestCase11() throws InterruptedException {
        ...
        assertTrue(status, "Advertisement 1 is clickable url is not match with actual url");
        ...
        status = !driver.getCurrentUrl().equals(currentURL);
        assertTrue(status, " Advertisement 2 is clickable url is not match with actual url ");
